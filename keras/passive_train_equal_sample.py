...
def main():
    ...
    parser.add_argument('--out', '-o', default='result/passive_equal/train13-53',
    ...
    parser.add_argument('--input2', type=int, default=53)
    ...
    if args.resume_model:
        print('loading..',args.resume_model)
    ...
    train_n, test_n = train_test_split(negative_index, train_size= np.sum(y==1), random_state=1211)
    #正例の抽出
    train_p = np.where(y==1)[0]

    #正例と負例のindexを混ぜてシャッフル
    index = np.append(train_p,train_n)
